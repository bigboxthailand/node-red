 ...
 *
 **/
const bodyParser = require('body-parser'); // โหลด body-parser ไว้ด้านบน
 ...
 ******************************************************************************/
    ...
    adminAuth: {
        type: "credentials",
        users: [{
            username: process.env.NODE_RED_EDITOR_USERNAME ,
            password: process.env.NODE_RED_EDITOR_PASSWORD ,
            permissions: "*"
        }]
    },
 ...
 ******************************************************************************/
    ...
    //                 next(err);
    //             } else {
    //                 req.rawBody = req.body.toString('utf8'); // เก็บ Raw Body ใน req.rawBody
    //                 next();
    //             }
    //         });
    //     } else {
    ...
    httpNodeMiddleware: (req, res, next) => {
        // ตรวจสอบ Content-Type ว่าเป็น application/json
        if (req.get('content-type') === 'application/json') {
            bodyParser.raw({ type: 'application/json' })(req, res, (err) => {
                if (err) {
                    next(err); // ส่งต่อข้อผิดพลาด
                } else {
                    // เก็บ Raw Body ไว้ใน req.rawBody
                    req.rawBody = req.body.toString('utf8');
                    next(); // ส่งต่อการทำงาน
                }
            });
        } else {
            next(); // ถ้าไม่ใช่ application/json ส่งต่อโดยตรง
        }
    },
 ...
 ******************************************************************************/
     ...
     */
    functionGlobalContext: {
        crypto: require('crypto') // เพิ่มบรรทัดนี้
        // os:require('os'),
    },
